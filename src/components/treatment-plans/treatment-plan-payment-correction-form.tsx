...
import {
  ...
  type RecordTreatmentPlanPaymentActionState,
...
type TreatmentPlanPaymentCorrectionFormProps = {
  treatmentPlanId: string
  payment: TreatmentPlanPaymentRow
  onSuccess?: (state: Extract<RecordTreatmentPlanPaymentActionState, { success: true }>) => void
}

/** "İade / Düzeltme" — always references the payment it corrects (`related_payment_id`), never edits the original row. Sprint 28D equivalent of `PaymentCorrectionForm`. */
function TreatmentPlanPaymentCorrectionForm({
  treatmentPlanId,
  payment,
  onSuccess,
}: TreatmentPlanPaymentCorrectionFormProps) {
  const [isPending, startTransition] = useTransition()
  const [formError, setFormError] = useState<string | null>(null)

  const form = useForm<TreatmentPlanPaymentCorrectionValues>({
    resolver: zodResolver(treatmentPlanPaymentCorrectionSchema),
    defaultValues: {
      treatmentPlanId,
  ...
  const handleSubmit = form.handleSubmit((values) => {
    ...
    startTransition(async () => {
      const result = await recordTreatmentPlanPaymentCorrection(values)
      if (result?.error) {
        setFormError(result.error)
        if (result.fieldErrors) {
          for (const [key, message] of Object.entries(result.fieldErrors)) {
            form.setError(key as keyof TreatmentPlanPaymentCorrectionValues, { message })
          }
        }
        return
      }
...
export { TreatmentPlanPaymentCorrectionForm }
